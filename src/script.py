...
class CollaborationController(nn.Module):
    ...
    def compute_collaboration_matrix(self, encoded_input, sim_matrix=None, shared_reps=None):
        if shared_reps is None:
            ...
            return torch.ones(B, self.num_models, self.num_models, device=encoded_input.device) / self.num_models

        routing_reps = self.collab_ffn(shared_reps)

        Q = self.W_Query(routing_reps) 
        K = self.W_Key(routing_reps)   
        
        C_logits = torch.matmul(Q, K.transpose(-2, -1)) / (self.hidden_dim ** 0.5)
    ...
    def forward(self, input_embedding, shared_reps=None, oracle_emb=None):
        ...
        if shared_reps is not None:
            ...
            C_soft = self.compute_collaboration_matrix(h, sim_matrix, shared_reps_hidden)
...
def prepare_datasets(args):
    if args.task == "mmlu":
        dataset = load_dataset(args.dataset, 'all', split="validation[:1%]")
    elif args.task == "mmlupro":
        dataset = load_dataset(args.dataset, split="validation")
    elif args.task == "gsm8k":
        dataset = load_dataset(args.dataset, 'main', split="test[:1%]")
    else:
...
def train_controller(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"Epoch {epoch+1}/{args.epochs}")):
            ...
            # Get shared representations for all models
            with torch.no_grad():
            ...
            # Generate outputs and compute losses
            batch_qualities = []
            final_outputs = []
            distillation_loss = 0
            ...
            for b in range(len(prompts)):
                ...
                for k in range(min(args.seq_len, len(topk_indices))):
                    ...
                    if EXPERT_TYPES != 'hosted':
                        ...
                        tokenizer = None  # No tokenizer for hosted models
...
def get_shared_embedding(texts, encoder, tokenizer, device=DEVICE):
    ...
    for text in texts:
        if not isinstance(text, str):
            text = str(text)
        cleaned_texts.append(text[:1000000])  # Limit to 1M chars to prevent OOM
...
def run_batch_collaborative_inference_selected(
    ...
    for i in range(B):
        ...
        for k, model_idx in enumerate(topk_indices):
            ...
            # Check for repetition if we have previous output
            if previous_output is not None and early_stopping:
                # Compare both string similarity and semantic similarity
                string_similarity = fuzz.ratio(output, previous_output)/100
                # emb1 = get_shared_embedding([output], encoder, tokenizer)
                # emb2 = get_shared_embedding([previous_output], encoder, tokenizer)
                # semantic_similarity = F.cosine_similarity(emb1, emb2).item()

                # If both similarities are above thresholds, stop early
                if string_similarity > 0.8:# and semantic_similarity > 0.9:
...
def inference(args):
    ...
    for step in tqdm(range(0, len(dataset), args.batch_size)):
        ...
        outputs, selection_info, selections, stopping_points, elapsed_time = run_batch_collaborative_inference_selected(
            ...
            device=DEVICE,
            top_k=controller.current_k # or threshold=0.2
        )
    ...
    with open(csv_file, mode="w", encoding="utf-8") as file:
        ...
        for i, prompt in enumerate(prompts):
            if args.task == "squad":
                ...
                ctx = prompt  # full prompt used as context
                q = dataset[i].get("question", "")
            elif args.task == "mmlupro":
                ctx = prompt  # full prompt used as context
                q = dataset[i].get("question", "")
